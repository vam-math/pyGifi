...
project_cone(target, data, basis, cone_type, ties, missing) -> np.ndarray
    Dispatch function: uses R's coneRegression() signature.
...
def _monotone_spline(target: np.ndarray, basis: np.ndarray,
    ...
    stronger constraint than the Dykstra approach and stays consistent with R's
...
def project_cone(
    ...
    Single dispatch entry-point: uses R's coneRegression() signature.
