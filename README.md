...
Gifi methods are a family of algorithms that find the best numerical representation of any kind of data, even if it is categorical or ordinal, so you can apply linear methods like PCA or regression to it. Each variable is **optimally transformed** to maximize structure.
...
- `pygifi.knots_gifi()`: B-spline knot placement with R-consistent output
...
- **Homals and Princals do not support out-of-sample projection**, just like in R: `model.transform(X)` returns the training-set object scores regardless of what `X` you pass: it does not raise an error, it simply ignores new data. Use `cv_morals()` for cross-validation instead.
...
- Results stay within floating-point tolerance of R's Gifi on every dataset covered by the parity suite (see [Validation Against R's Gifi](#️-validation-against-rs-gifi)); two known, explained exceptions are documented in `tests/parity/README.md`.

---

## Validation Against R's Gifi
...
The project includes one file per gifi algorithm, the modules required for transformation of the data, cross validation and other utilities functions. 
...
The R to python conversion was developed by Vamanie Perumal, Indian Institute of Technology Madras and Sanjeev, Bhaavesh from Amrita College of Engineering, Chennai Campus.
